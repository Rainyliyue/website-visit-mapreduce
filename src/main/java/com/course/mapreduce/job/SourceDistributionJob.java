...
public class SourceDistributionJob {
    public static class SourceMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            outKey.set(record.regionKey());
            context.write(outKey, ONE);
            outKey.set(record.ipKey());
            context.write(outKey, ONE);
        }
    }

    public static Job createJob(Configuration configuration, String input, String output) throws IOException {
        Job job = Job.getInstance(configuration, "website source distribution");
        job.setJarByClass(SourceDistributionJob.class);
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: SourceDistributionJob <input> <output>");
            System.exit(2);
        }
